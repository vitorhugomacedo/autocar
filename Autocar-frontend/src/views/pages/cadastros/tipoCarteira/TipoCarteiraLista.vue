  ...
  methods: {
    showTemplate: function (id, message) {
      this.confirm.require({
        ...
        acceptLabel: 'Excluir',
        accept: async () => {
          await this.tipoCarteiraService.delete({ id })
    ...
    searchRecords: async function () {
      ...
      this.tipoCarteiraService = new TipoCarteiraService();
      this.registros = await this.tipoCarteiraService.getAll(this.urlService.getQueryParamsObject());
...
<template>
  <div class="font-medium text-2xl text-color">Tipos de Carteiras</div>
    ...
    <div class="col-12 md:col-12">
      <div class="card">
        <Toolbar class="mb-4">
          <template v-slot:end>
            <div class="my-2">
              <router-link :to="'/financeiro/cadastros/tipo-carteira/novo'">
        ...
        <DataTable :value="registros.data" paginator lazy :first="(registros.currentPage - 1) * registros.pageSize"
          ...
          <template v-if="!loading" #empty>Nenhum tipo de carteira encontrado. </template>
          <template #header>
          ...
          <Column field="Edit" header="" dataType="boolean" bodyClass="text-right" style="min-width: 4rem">
            <template #body="{ data }">
              <div v-if="data.editavel">
                <router-link :to="`/financeiro/cadastros/tipo-carteira/editar/${data.id}`">
                ...
                <Button v-tooltip.top="'Excluir'" icon="pi pi-trash" class="p-button-rounded p-button-text" text rounded
                  @click="showTemplate(data.id, `Deseja realmente excluir o tipo de carteira ${data.descricao}?`)"></Button>
              </div>
            </template>
          </Column>
