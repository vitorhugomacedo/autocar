...
export default {
  ...
  data() {
    return {
      ...
      breadcrumbItems: [{
        label: 'Configurações',
        to: '/configuracoes/perfil-usuario'
      },
      {
  ...
  methods: {
    showTemplate: function (id, message) {
      this.confirm.require({
        ...
        acceptLabel: 'Excluir',
        accept: async () => {
          await this.perfilUsuarioService.delete({ id })
    ...
    searchRecords: async function () {
      ...
      this.registros = await this.perfilUsuarioService.getAll(this.urlService.getQueryParamsObject());
    ...
    buildFilters: async function () {
      this.filters = [
        { field: 'Descrição', property: 'descricao', type: 'text' }
      ]
    },
...
<template>
  <div class="font-medium text-2xl text-color">Perfil de Usuário</div>
    ...
    <div class="col-12 md:col-12">
      <div class="card">
        <Toolbar class="mb-4">
          <template v-slot:end>
            <div class="my-2">
              <router-link :to="'/configuracoes/perfil-usuario/novo'">
        ...
        <DataTable :value="registros.data" paginator lazy :first="(registros.currentPage - 1) * registros.pageSize"
          ...
          <template v-if="!loading" #empty>Nenhum perfil de usuário encontrado. </template>
          <template #header>
          ...
          <Column field="id" header="Cód." bodyClass="text-left" style="min-width: 5rem">
            <template #body="{ data }">
              <span style="vertical-align: middle" class="image-text">{{ data.id }}</span>
            </template>
          </Column>
          <Column field="descricao" header="Nome" style="min-width: 8rem">
            <template #body="{ data }">
              {{ data.descricao }}
            </template>
          </Column>
          <Column field="Edit" header="" dataType="boolean" bodyClass="text-right" style="min-width: 10rem">
            <template #body="{ data }">
              <router-link :to="`/configuracoes/perfil-usuario/editar/${data.id}`">
                <Button v-tooltip.top="'Editar'" icon="pi pi-user-edit" class="p-button-rounded p-button-text mr-4" text
                  rounded></Button>
              </router-link>

              <Button v-tooltip.top="'Excluir'" icon="pi pi-trash" class="p-button-rounded p-button-text" text rounded
                @click="showTemplate(data.id, `Deseja realmente excluir o perfil ${data.descricao}?`)"></Button>
            </template>
          </Column>
